...
from basic_env_v3 import one_dim_control 
from q_agent_v1 import Agent_v1 
from rocket_env_render import Render_Rocket

env = one_dim_control() 
agent = Agent_v1(LEARNING_RATE=0.1, DISCOUNT=0.95, EPS0=0.5, EPS_DEC_START=0.1, EPS_DEC_STOP=0.8)

agent.prep_agent(env.observation_space['low'], env.observation_space['high'], env.action_space['n'], DISCRETE_OS_SPACE=50, q_init=(100, 400))
q_table, info, q_tables = agent.train(env, EPISODES=1000, SHOW_EVERY=500)
performance = agent.use_agent(env, render=True)
agent.save_agent("agent_rocket_v3")
